        ...
        values: [
          [
            ...
            game.platforms && Array.isArray(game.platforms)
              ? game.platforms.map((p) => p.name.replace(/,/g, '-%-')).join('-%-')
            ...
            game.involved_companies && Array.isArray(game.involved_companies)
              ? game.involved_companies.some((c) => c.developer)
                ? game.involved_companies.filter((c) => c.developer && c.company?.name).map((c) => c.company.name.replace(/,/g, ' ')).join('-%-')
                : game.involved_companies.map((c) => c.company?.name?.replace(/,/g, ' ')).filter(Boolean).join('-%-')
              : game.developers_string || '',
            game.involved_companies && Array.isArray(game.involved_companies)
              ? game.involved_companies.some((c) => c.publisher)
                ? game.involved_companies.filter((c) => c.publisher && c.company?.name).map((c) => c.company.name.replace(/,/g, ' ')).join('-%-')
                : game.involved_companies.map((c) => c.company?.name?.replace(/,/g, ' ')).filter(Boolean).join('-%-')
